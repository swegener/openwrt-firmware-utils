...
static void fill_header(char *buf, int len)
{
	...
	hdr->size1 = htonl(len - offsetof(struct fw_header, version));
	hdr->size2 = htonl(sizeof(struct fw_header) - offsetof(struct fw_header, version));
	hdr->size3 = htonl(len - sizeof(struct fw_header));

	memcpy(hdr->md5sum2, md5salt, sizeof(hdr->md5sum2));
	get_md5(hdr->md5sum2, MD5SUM_LEN + 32768, hdr->md5sum2);
	memcpy(hdr->md5sum1, md5salt, sizeof(hdr->md5sum1));
	get_md5(hdr->md5sum1, MD5SUM_LEN + 32768, hdr->md5sum1);
}
